    ...
    <div className="bg-white dark:bg-stone-900 p-4 sm:p-6 rounded-xl border border-stone-200 dark:border-stone-800 shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-4 sm:gap-0">
        <h2 className="text-lg font-bold text-stone-900 dark:text-stone-100">
          Hostel Images ({images.length}/{maxImages})
        </h2>
        ...
        <div
          className={`border-2 border-dashed rounded-xl p-6 sm:p-8 text-center transition-colors ${
            dragActive
              ...
              : "border-stone-300 dark:border-stone-700 hover:border-orange-300 hover:bg-stone-50 dark:hover:bg-stone-800"
          }`}
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-stone-600 dark:text-stone-400 text-sm sm:text-base leading-relaxed">
              ...
              <span
                ...
                className="text-orange-600 cursor-pointer font-medium hover:text-orange-700 dark:text-orange-500 dark:hover:text-orange-400"
              >
                Browse
              </span>
            </p>
            <span className="text-xs text-stone-500 px-3 py-1 bg-stone-100 dark:bg-stone-800 rounded-full">
              Max 5MB per image
            </span>
          </div>
        </div>
      ...
      {images.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 mt-6">
          {images.map((img, idx) => (
            <div
              key={img.id}
              className={`relative group rounded-lg overflow-hidden border-2 h-24 sm:h-32 ${
            ...
            >
              ...
              {selectionMode && (
                ...
                >
                  {selectedImages.has(img.id) ? (
                    ...
                    <Square
                      size={16}
                      className="text-stone-400 group-hover:text-stone-600 dark:group-hover:text-stone-300"
                    />
                  )}
                </button>
              )}
              ...
              {!selectionMode && img.isCover && (
                <span className="absolute top-2 left-2 bg-orange-600 text-white text-xs px-2 py-1 rounded shadow-sm font-medium">
                  Cover
                </span>
              )}
              {!selectionMode && !img.isExisting && (
                <span className="absolute top-2 right-2 bg-emerald-500 text-white text-xs px-2 py-1 rounded shadow-sm font-medium">
                  New
                </span>
              )}
