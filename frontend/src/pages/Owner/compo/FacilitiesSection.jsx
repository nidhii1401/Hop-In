      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          {["NONE", "OPTIONAL", "COMPULSORY"].map((type) => (
            <label
              key={type}
              className={`cursor-pointer border p-3 rounded-lg flex items-center justify-center gap-2 font-medium transition-all ${
                formData.messType === type
            ...
            >
              <input
                ...
                name="messType"
                value={type}
                checked={formData.messType === type}
                onChange={(e) => handleChange({ messType: e.target.value })}
                className="hidden"
              />
              {type}
            </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleChange({ messPricePerMonth: e.target.value })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleChange({ messDescription: e.target.value })
                }
                className="w-full p-2.5 rounded-lg border border-stone-300 dark:border-stone-700 bg-transparent"
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-sm border transition-colors text-left ${
                formData.amenities.includes(amenity.id)
                  ? "bg-orange-600 text-white border-orange-600 font-medium shadow-md"
                  : "bg-white text-stone-600 border-stone-300 hover:border-stone-400 hover:bg-gray-300 hover:text-black"
              }`}
            >
              {amenity.name}
            </button>
